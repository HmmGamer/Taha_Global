...
public class UiRayCastDebugger : MonoBehaviour
{
    ...
    private void ProcessResults()
    {
        if (eventSystem == null)
        {
            Debug.Log("[UI Raycast] No EventSystem found in scene");
            return;
        }
        ...
        {
            report.AppendLine("[UI Raycast] No UI elements hit by raycast");
            ...
            foreach (var group in allCanvasGroups)
            {
                ...
                {
                    report.AppendLine($"?? Potential blocker: CanvasGroup '{group.gameObject.name}' has blocksRaycasts = false");
    ...
    private bool AppendButtonInfo(GameObject obj, StringBuilder report)
    {
        ...
        {
            report.AppendLine("?? PROBLEM: Button has no click handlers");
            hasProblems = true;
        }
    ...
    private bool AppendCanvasGroupInfo(GameObject obj, StringBuilder report)
    {
        ...
        if (groups.Length > 0)
        {
            foreach (var group in groups)
            {
                ...
                {
                    report.AppendLine($"?? PROBLEM: CanvasGroup {group.gameObject.name} is not interactable");
                    hasProblems = true;
                }

                if (!group.blocksRaycasts)
                {
                    report.AppendLine($"?? PROBLEM: CanvasGroup {group.gameObject.name} does not block raycasts");
                    hasProblems = true;
                }
            }
        }
