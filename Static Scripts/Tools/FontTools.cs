...
public static class FontTools
{
    ...
    public static string _ConvertToPersian(string iText)
    {
        ...
        var finalOutput = new List<char>(iText.Length);
        ...
        for (int i = 0; i < iText.Length; i++)
        {
            ...
            if (joinPrev && joinNext) currentLine.Add(g.Med);
            else if (joinPrev) currentLine.Add(g.Fin);
            else if (joinNext) currentLine.Add(g.Ini);
            else currentLine.Add(g.Iso);
        }

        finalOutput.AddRange(currentLine);

        return new string(finalOutput.ToArray());
    }
    public static string _ConvertToPersian_Visual(string iText)
    {
        if (string.IsNullOrEmpty(iText))
            return string.Empty;
    ...
    public static string _ConvertPersianToVisual(string iText)
    {
        if (string.IsNullOrEmpty(iText))
            return string.Empty;
